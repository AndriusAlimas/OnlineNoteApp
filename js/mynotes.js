...
$(document).ready(function () {
    ...
    var editMode = false;
            ...
            $('#addNote').click(function () {
                $.ajax({
                   url: "createnote.php",
                    success: function (data) {
                      // if you was on edit mode, and press AddNote, just make sure you now not in edit mode
                       editMode = false;
                        if(data == 'error'){
                            $('#alertContent').text("There was an issue inserting the new note in the database!");

                            $("#alert").fadeIn();
                        }else{ // ajax call back we get id from echo mysqli_insert_id($connect); this is data
                            // update activeNote to the id of the new note
                            activeNote = data;
                          // make sure it's empty note
                            $("textarea").val("");
                            // show hide elements
            ...
            $("textarea").keyup(function () {
                    // ajax call to updatenote.php the task of id activenote
                    $.ajax({
                       url: "updatenote.php",
                       type: "POST", // this time we want send data using POST method
                       // we need to send the current note content with its id to the php file
                       data: {  note: $(this).val() ,
                                id: activeNote // this is content , received by post method
                           },
                        success: function (data) {
                            if(data == 'error'){
   ...
   // show Hide function
    function showHide(arrayShow, arrayHide) {
